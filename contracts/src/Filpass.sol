 ...
 * @dev
 * This contract facilitates the secure deposit, withdrawal, and refund of FIL (the native currency of the Filecoin network) between a single user and multiple oracles on the Filecoin network.
 *
 * Features:
 * - **Deposits:** The user can deposit FIL specifying an Oracle and a Recipient, along with a lock-up period in days.
 *   - **First Deposit:** For each Oracle-Recipient pair, the first deposit extends the lock-up time by an additional day.
 *   - **Subsequent Deposits:** Updates the refund time to the maximum of the existing refund time and the new calculated refund time based on the latest deposit.
 *   - **Limits:** The contract supports a maximum of 10 Oracles, and each Oracle can have a maximum of 10 Recipients to prevent gas limit issues.
 * - **Withdrawals:** Only the designated Oracle can withdraw funds for their associated Recipient **before** the refund time.
 * - **Refunds:** The user can request refunds based on different scenarios:
 *   - **Specific Oracle-Recipient Pair:** Refund for a specific Oracle and Recipient after the refund time has elapsed.
 *   - **All Recipients for an Oracle:** Refund all available funds for a specific Oracle across all Recipients after their respective refund times have elapsed.
 *   - **All Oracles and Recipients:** Refund all available funds across all Oracles and Recipients after their respective refund times have elapsed.
 *     - When refunding all, any additional FIL balance in the contract is also transferred back to the user.
 * - **Emergency Withdrawal:** The user can recover any stranded FIL in the contract that may have been sent accidentally or via other means.
 *
 * Security Measures:
 * - **Access Control:** Ensures that only the user can perform deposits, refunds, and emergency withdrawals, while only the designated Oracle can perform withdrawals.
 * - **Reentrancy Protection:** Utilizes OpenZeppelin's `ReentrancyGuard` to prevent reentrancy attacks.
 * - **Proper Fund Handling:** Uses low-level `call` for transferring FIL, handling dynamic gas requirements and potential contract recipients.
 * - **Event Logging:** Emits events for deposits, withdrawals, refunds, and the removal of Oracles and Recipients to facilitate off-chain tracking and auditing.
 * - **Input Validation:** Validates all inputs to prevent invalid states, unauthorized access, and potential overflows/underflows.
 * - **Fallback Mechanisms:** Prevents direct FIL transfers to the contract, enforcing usage of designated functions for deposits.
 * - **Limitations:** Enforces a maximum number of Oracles and Recipients per Oracle to prevent gas limit issues during bulk operations.
 */
...
contract FilecoinDepositWithdrawRefund is ReentrancyGuard {
     ...
     * @return DepositInfo struct containing the deposited amount and refund time.
     */
    mapping(address => mapping(address => DepositInfo)) public deposits;

    /// @notice Array to keep track of all Oracles.
     ...
     *
     * Behavior:
     * - **First Deposit:** Extends the lock-up time by an additional day.
     * - **Subsequent Deposits:** Updates the refund time to the later of the existing refund time or the new calculated refund time.
     * - **Limits:** Enforces a maximum of 10 Oracles and 10 Recipients per Oracle to prevent gas limit issues.
     *
     * Requirements:
     * - `oracleAddress` and `recipient` must be valid non-zero addresses.
     * - `lockUpTime` must be a positive integer representing days and not exceed `MAX_LOCKUP_DAYS`.
     * - The deposit amount (`msg.value`) must be greater than zero.
     * - The total number of Oracles must not exceed `MAX_ORACLES`.
     * - The total number of Recipients under an Oracle must not exceed `MAX_RECIPIENTS_PER_ORACLE`.
     *
    ...
    function depositAmount(
        ...
        address recipient,
        uint256 lockUpTime
    ) external payable onlyUser nonReentrant {
     ...
     *
     * Behavior:
     * - The Oracle can withdraw a specified amount of FIL to the designated Recipient.
     * - The withdrawal can only occur **before** the refund time.
     *
     * Requirements:
     * - `recipient` must be a valid non-zero address.
     * - The Oracle must have sufficient deposited funds for the Recipient.
     * - The current time must be **before** the refund time.
     *
     * @param recipient The address of the Recipient to receive the withdrawn funds.
     * @param withdrawAmount The amount of FIL to withdraw.
     */
    function withdrawAmount(
        ...
        require(info.amount >= withdrawAmount, "Insufficient funds");
     ...
     *
     * Behavior:
     * - **Specific Oracle-Recipient Pair:** If both `oracleAddress` and `recipient` are provided, refunds funds for that pair after the refund time.
     * - **All Recipients for an Oracle:** If only `oracleAddress` is provided and `recipient` is the zero address, refunds all available funds for that Oracle across all Recipients after their respective refund times have elapsed.
     * - **All Oracles and Recipients:** If both `oracleAddress` and `recipient` are zero addresses, refunds all available funds across all Oracles and Recipients after their respective refund times have elapsed.
     *   - When refunding all, any additional FIL balance in the contract is also transferred back to the user.
     *
     * Requirements:
     * - For bulk refunds, the total number of Oracles and Recipients is limited to prevent gas limit issues.
     *
    ...
    /**
     * @dev Internal function to refund all available funds across all Oracles and Recipients.
     *      Also transfers any remaining FIL balance in the contract back to the user.
     */
    ...
    /**
     * @dev Internal function to remove a Recipient from an Oracle's list when their balance reaches zero.
     * @param oracleAddress The address of the Oracle.
     * @param recipient The address of the Recipient.
     */
    ...
    /**
     * @dev Internal function to remove an Oracle from the list when they have no Recipients left.
     ...
     *      This function can be used to withdraw any FIL that was sent to the contract accidentally or via other means.
     *
     * Requirements:
     * - Only the user can call this function.
     * - The contract must have a FIL balance greater than zero.
     */
     ...
     *      Enforces that all deposits must go through the `depositAmount` function.
     *      Reverts any direct transfers with a descriptive error message.
     */
