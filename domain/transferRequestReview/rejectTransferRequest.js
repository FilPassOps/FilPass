...
export async function batchRejectTransferRequest(params) {
  ...
  const { data, error } = await newPrismaTransaction(async fnPrisma => {
    const promiseList = transferRequests.map(async tRequest => {
      ...
      if (!currentProgram) {
        throw new TransactionError(errorsMessages.transfer_request_not_found.message, { status: 404 })
      }
    ...
    if (data.find(req => req.status === 'rejected')) {
      const [reason] = data.find(req => req.status === 'rejected')
      throw { ...reason }
    }
  ...
  const promiseList = data.map(async tRequest => {
    const currentProgram = tRequest && approverPrograms.find(({ program }) => program.id === tRequest.programId)
    if (currentProgram?.program?.visibility !== PROGRAM_TYPE_INTERNAL) {
