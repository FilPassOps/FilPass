...
describe('getApproved', () => {
  it('Should return paginated approved transfer requests', async () => {
    ...
    expect(getApproved).not.toThrowError()
  })
  ...
  it('Should return sorted and orders transfer request', async () => {
    ...
    const { data: firstData } = await getApproved(params)
    ...
    const { data: secondData } = await getApproved(params2)
