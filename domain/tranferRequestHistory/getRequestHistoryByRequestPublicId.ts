...
interface GetRequestHistoryByRequestPublicIdParams {
...
export async function getRequestHistoryByRequestPublicId(params: GetRequestHistoryByRequestPublicIdParams) {
  ...
  const history = await prisma.$queryRaw<History[]>`
    SELECT history.id,
        history.created_at,
        history.field,
        app_user.email,
  ...
  const deryptedHistory = await Promise.all(
    history.map(async (row: { field: string; email: string; old_value: string; new_value: string }) => {
