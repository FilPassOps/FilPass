...
export const getUserTransactionCreditsByUserId = async (props: GetUserTransactionCreditsByUserIdParams) => {
  try {
    ...
    SELECT * FROM (
      SELECT 'DEPOSIT' as type, ct.id, ct.transaction_hash, ct.status, ct.amount, ct.created_at, sp.wallet_address
      ...
      SELECT 'REFUND' as type, rt.id, rt.transaction_hash, rt.status, rt.amount, rt.created_at, sp.wallet_address
      ...
      SELECT 'WITHDRAW' as type, wt.id, wt.transaction_hash, wt.status, wt.amount, wt.created_at, sp.wallet_address
