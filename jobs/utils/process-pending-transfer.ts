...
export async function processPendingTransfer(chain: Chain, contract: MultiForwarder) {
  ...
  await sendBatchEmail({
    ...
    html: baseEmail(getBody(failedTransactions, failedTransferRequestPublicIds, chain.blockExplorer.url)),
  })
}

const getBody = (failedTransactions: string[], failedTransferRequestPublicIds: string[], blockExplorerUrl: string) => {
  // prettier-ignore
  return `
  <tr>
