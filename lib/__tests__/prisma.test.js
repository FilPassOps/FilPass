...
describe('prisma lib module', () => {
  describe('newPrismaTransaction', () => {
    ...
    it('should return error message when when it is not a prisma error', async () => {
      ...
      expect(isPrismaErrorSpy).toBeCalledWith(
        new TransactionError('error message', { status: 424 })
      )
    })
  ...
  describe('reportTransactionException', () => {
    it('should call sentry when received error does not have status', () => {
      const error = {
        message: 'error_message',
      }

      prismaModule.reportTransactionException(error)

      expect(sentry.captureException).toBeCalledWith({ message: 'error_message' })
    })

    it('should call sentry when received error status is = 500', () => {
      const error = {
        status: 500,
        message: 'error_message',
      }
      ...
      expect(sentry.captureException).toBeCalledWith({ status: 500, message: 'error_message' })
    })
    ...
    it('should call sentry when received error status is > 500', () => {
      ...
      expect(sentry.captureException).toBeCalledWith({ status: 503, message: 'error_message' })
    })

    it('should call sentry when received error status is < 400', () => {
      const error = {
        status: 200,
        message: 'error_message',
      }
      ...
      expect(sentry.captureException).toBeCalledWith({ status: 200, message: 'error_message' })
    })

    it('it not should call sentry when received error status is < 500 and > 400', () => {
      const error = {
        status: 404,
        message: 'error_message',
      }
