...
export const UserList = ({ data = [], loggedUser = {} }) => {
  ...
  const [selectedUserViewReason, setSelectedUserViewReason] = useState()
  ...
  const handleOnViewClick = reason => {
  ...
  return (
    <div className="flex flex-col relative">
          ...
          {data.map(user => (
            <tr key={user.id}>
              ...
              <Cell className={classNames('break-all w-full', scrolled && 'relative', !scrolled && 'absolute')} style={{ maxWidth: 300 }}>
                <SelectRoles user={user} scrolled={scrolled} loggedUser={loggedUser} />
              </Cell>
      ...
      <BanUserModal open={openBanModal} onModalClosed={() => setOpenBanModal(false)} user={selectedUser} />
      <UnbanUserModal open={openUnbanModal} onModalClosed={() => setOpenUnbanModal(false)} user={selectedUser} />
