...
export const BatchCSV = () => {
  ...
  const {
    ...
    resolver: yupResolver(yup.object({ programId: yup.number().integer().positive().max(MAX_INTEGER_VALUE).required() })),
    defaultValues: {
      programId: '',
      currencyUnitId: '',
    },
  ...
  const handleUploadCSV = async file => {
    ...
    const { error, data } = await api.postForm('/transfer-requests/upload-csv', formData)
...
const ModalComponent = ({ openModal, onModalClosed, errors = [] }) => {
  return (
    <Modal
      open={openModal}
      onModalClosed={() => {
...
const OpenFile = ({ file }) => {
  const handleOpenFile = async () => {
    ...
    const objectURL = URL.createObjectURL(blob, { type: 'text/csv' })
    newWindow.location = objectURL
  }
  return (
    <div className="flex items-center">
      <DocumentTextIcon className="w-6 h-6 mr-2" />
      <button type="button" className="text-sm text-indigo-500 font-bold hover:underline cursor-pointer" onClick={handleOpenFile}>
        {file?.originalname}
      </button>
    </div>
  )
